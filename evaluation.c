...
int main(int argc, char** argv) {
    ...
    mpc_parser_t* OprChar  = mpc_new("oprchar");
    ...
    mpca_lang(MPCA_LANG_DEFAULT,
        "                                                       \
            number   :  /-?[0-9]+/ ;                            \
            oprchar  : /add/ | /sub/ | /mul/ | /div/ | /mod/ | '^' | /pow/ | /min/ | /max/ | ;            \
            operator : '+' | '-' | '*' | '/' | '%' | <oprchar> ;\
            expr     : <number> | '(' <operator> <expr>+ ')' ;  \
            lispy    : /^/ <operator> <expr>+ /$/ ;             \
        ",
        Number, OprChar, Operator, Expr, Lispy
    );
    ...
    mpc_cleanup(5, Number, OprChar, Operator, Expr, Lispy);
